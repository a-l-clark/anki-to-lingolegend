...
A browser-based tool (`index.html`, `styles.css`, `script.js`) that converts an Anki deck export (`.apkg`) into a CSV formatted for the Lingo Legend custom flash cards template. Everything runs client-side — your deck never leaves your browser.
...
- **Topic details step** — set Topic Description and Topic Type once per topic (matching the template's rule that these only need to appear on a topic's first row).
  - Topic Type is a fixed dropdown of Lingo Legend's allowed values: `0 - Vocab`, `1 - Grammar`, `2 - Phrases`, `3 - Script`, `4 - Mixed`.
  ...
  - **Drag to reorder topics** using the ⠿ handle on each row — this also sets the order topics appear in the exported CSV, and your custom order is preserved even if you go back and revisit earlier steps.
...
2. Keep `index.html`, `styles.css`, and `script.js` together in the same folder, then open `index.html` in a browser (just double-click it — no server or install needed).
...
6. Fill in Topic Description / Topic Type per topic (use "apply to all" for Topic Type, and drag rows to reorder topics if you want a specific order in the CSV).
...
| File | Purpose |
|---|---|
| `index.html` | Page structure and step layout |
| `styles.css` | All styling |
| `script.js` | Parsing, mapping, validation, and export logic |
